...
`.agents/skills/maintain-nexus-codex/`. Its references contain the operational
...
The initial registry bootstrap is complete. `@nexus-agent-x/codex` exists,
npm Trusted Publishing is configured for `NexusAgentX/codex`, workflow
`nexus-release.yml`, environment `npm-publish`, and the repository variable
`NEXUS_NPM_PUBLISH_ENABLED` is enabled. Normal tagged releases must use the
OIDC job and must not request or store an npm password, OTP, or long-lived
token.

### First registry release (completed)

The commands below document the one-time bootstrap that was already completed.
Do not repeat them for normal releases or recovery. They apply only if the npm
package must be recreated, which requires separate explicit user authorization.
...
The configured npm Trusted Publisher contract is:
...
The matching GitHub environment and repository variable must remain present.
Verify them without mutating release configuration:
...
The tag workflow validates the complete set again, publishes all six
platform versions serially, publishes the root under `latest`, and verifies
every dist-tag. It uses no `NODE_AUTH_TOKEN` or stored npm secret.
